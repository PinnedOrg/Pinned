...
const slursOrSexualContent = [
    'nigger', 'nigga', 'sex', 'chink', 'fuck', 'rape', 'cunt', 'faggot' // replace with actual slurs/sexual terms
];

const mildProfanity = ['shit', 'hell', 'idiot', 'stupid', 'crap'];
...
function isTextProfane(text) {
    ...
    for (const word of words) {
        if (engine.isProfane(word)) {
            if (mildProfanity.includes(word)) {
