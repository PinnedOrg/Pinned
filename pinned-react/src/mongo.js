...
async function main(){
    ...
    const uri = 'mongodb+srv://dmruds123:Radius335841482@pinneddb.ulvjrsw.mongodb.net/?retryWrites=true&w=majority';    //connecter uri
...
async function createEvent(client, newEvent){
    const result = await client.db("Pinned").collection("event").insertOne(newEvent);
}

//creates multiple events
async function createEvents(client, newEvents){ //newEvents should be an array of event objects
    const result = await client.db("Pinned").collection("event").insertMany(newEvents);
}

//read an event: returns event object
async function findEvent(client, nameOfEvent){
    const result = await client.db("Pinned").collection("event").findOne({Title:nameOfEvent});
    return result;
...
async function updateEvent(client, nameOfEvent, updatedEvent){
    const result = await client.db("Pinned").collection("event").updateOne({name: nameOfEvent}, {$set: updatedEvent});
}

//deletes event from database
async function deleteEvent(client, nameOfEvent){
    const result = await client.db("Pinned").collection("event").deleteOne({Title:nameOfEvent});
}
